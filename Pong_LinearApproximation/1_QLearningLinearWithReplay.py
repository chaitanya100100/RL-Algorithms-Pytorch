...
# linear approximation function as torch network
...
env = wrappers.Monitor(env, "./tmp_qlearning_replay", force=True)

eps_scheduler = LinearSchedule(explore_till, eps)
replay_buffer = ReplayBufferSingle(capacity=replay_buffer_size)

qnet = LinearNet(env.observation_space.shape[0], env.action_space.n).to(device)
...
for t in itertools.count():
    ...
    q_val = qnet(torch.from_numpy(obs).type(torch.float).unsqueeze(0).to(device))[0]
    ...
    # take action and store the transition in replay buffer
    ...
    # ---------------------------
    # Update on random samples from replay buffer
    # ---------------------------
    ...
    # estimate the q values and calculate error
    q_val = qnet(b_obs)
    next_q_val = qnet(b_next_obs)
    target = (b_reward + gamma * next_q_val.max() * (1-b_done) ).detach()
    #loss = ((target - q_val.gather(1, b_action.unsqueeze(1)))**2).sum()
    loss = criterion(target, q_val.gather(1, b_action.unsqueeze(1)).squeeze())
...
import pickle
with open("QL_with_replay.pkl", "wb") as f:
...
plt.title("Pong Linear Q-Learning with Replay Buffer")
plt.xlabel("Episodes")
plt.ylabel("Rewards")
plt.show()
