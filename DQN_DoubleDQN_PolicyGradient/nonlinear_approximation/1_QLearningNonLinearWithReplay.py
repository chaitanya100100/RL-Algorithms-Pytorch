...
env = wrappers.Monitor(env, "./tmp_qlearning_nonlinear_replay", force=True)

eps_scheduler = LinearSchedule(explore_till, eps)
replay_buffer = ReplayBufferSingle(capacity=replay_buffer_size)

qnet = QNet(env.observation_space.shape[0], env.action_space.n).to(device)
...
import pickle
with open("QL_nonlinear_with_replay.pkl", "wb") as f:
...
plt.title("Pong Nonlinear Q-Learning with Replay Buffer")
plt.xlabel("Episodes")
plt.ylabel("Rewards")
plt.show()
