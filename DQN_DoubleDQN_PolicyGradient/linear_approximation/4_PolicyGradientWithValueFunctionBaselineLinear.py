...
class ValueFunctionNetworkLinear(nn.Module):
    def __init__(self, num_inp):
        super(ValueFunctionNetworkLinear, self).__init__()
        self.fc = nn.Linear(num_inp, 1)
...
value_estimator = ValueFunctionNetworkLinear(env.observation_space.shape[0])
...
import pickle
with open("Pong_PG_linear_with_baseline.pkl", "wb") as f:
...
plt.title("Pong Linear Policy Gradient with Value function Baseline")
plt.xlabel("Episodes")
plt.ylabel("Rewards")
plt.show()
