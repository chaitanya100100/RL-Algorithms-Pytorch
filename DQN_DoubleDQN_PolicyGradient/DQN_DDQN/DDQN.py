...
def double_q_learning_fun(
    ...
    #Q.load_state_dict(torch.load("./models/best_model.pth"))
    #target_Q.load_state_dict(torch.load("./models/best_model.pth"))
    #print("Loaded saved dict")
    ...
    for t in count():
        ...
        if (t > learning_starts and
            ...
            ## DDQN
            next_max_Q_values_idx = Q(next_obs_batch).max(1)[1].detach().unsqueeze(1)
            next_max_target_Q_values = target_Q(next_obs_batch).gather(1, next_max_Q_values_idx).squeeze()
        ...
        # Log progress and save statistics
        ...
        if t % LOG_EVERY_N_STEPS == 0 and t > learning_starts:
            ...
            with open('./ddqn/statistics.pkl', 'wb') as f:
                pickle.dump(Statistic, f)
                print("Saved to %s" % './ddqn/statistics.pkl')

            if save_best_mean_reward < best_mean_episode_reward:
                save_best_mean_reward = best_mean_episode_reward
                torch.save(Q.state_dict(), './ddqn/best_model.pth')


        if t % SAVE_EVERY_N_STEPS == 0:
            torch.save(Q.state_dict(), './ddqn/n_steps_%d.pth' % t)
