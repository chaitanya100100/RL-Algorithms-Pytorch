...
# network for linear approximation
...
env = wrappers.Monitor(env, "./tmp_doubleqlearning_replay", force=True)
...
qnet1 = LinearNet(env.observation_space.shape[0], env.action_space.n).to(device)
qnet2 = LinearNet(env.observation_space.shape[0], env.action_space.n).to(device)
...
import pickle
with open("DoubleQL_with_replay.pkl", "wb") as f:
...
plt.title("Pong Linear Double Q-Learning with Replay Buffer")
plt.xlabel("Episodes")
plt.ylabel("Rewards")
plt.show()
